...
davidson.16.monthly <- davidson.16 %>% 
  ...
  group_by(year, month, site_position ) %>%
...
davidson.16.monthly <- davidson.16.monthly %>% 
  mutate(veg_detail = case_when(site_reference %in% c("BEO_flat_centre","BEO_rim")~ "Dry graminoid",
                               site_reference %in% "BEO_high_centre" ~ "Moss-lichen",
                               site_reference %in% c("BEO_low_centre", "BEO_trough", "BES_drained_lake",
                                                     "IVO_wetland" , "ATQ_pool") ~ "Wet sedge",
                               site_reference %in% "IVO_plateau_hollow" ~ "Moss only",
                               site_reference %in% c("IVO_plateau_inter_tussock", "ATQ_ridge_inter_tussock")~ "Moss-shrub",
                               site_reference %in% c("IVO_plateau_tussock", "ATQ_ridge_tussock")~ "Tussock sedge"))
davidson.16.monthly <- davidson.16.monthly %>% 
  mutate(veg_detail = case_when(site_reference %in% "BEO_flat_centre"~ "UpTundra",
...
davidson.19.monthly <- davidson.19 %>% 
  ...
  group_by(year, month, site_reference, Site, ) %>%
...
davidson.19.monthly$site_id <- paste("PoplarFen", davidson.19.monthly$site_reference, "agg", sep = "_")
...
PIdat.ch <- rbindlist(list(peacock, jung, althuizen,  jassey, sabrekov.ch, dyukarev.ch, hung.ch,
                            davidson.16.monthly, davidson.19.monthly ), 
                      fill = TRUE)
